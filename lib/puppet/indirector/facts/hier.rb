...
class Puppet::Node::Facts::Hier < Puppet::Node::Facts::Facter
  ...
  def find(request)
    ...
    super.values.reject { |k, v| Symbol == k.class }.each &make_hier
    Puppet::Node::Facts.new(request.key, hier)
  end

  def destroy(facts); end
